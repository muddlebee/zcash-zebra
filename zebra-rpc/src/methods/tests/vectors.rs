...
async fn rpc_getaddressutxos_invalid_arguments() {
    ...
    let error = rpc.get_address_utxos(addresses).await.unwrap_err();
    assert_eq!(
        error.message,
...
async fn rpc_getaddressutxos_response() {
    ...
    // TODO: We expect the response to be at least 1?
    // Confirm and finish the test in the context of #3158
