...
fn spawn_lightwalletd_with_rpc_server(
    ...
    let mut arguments = vec!["--grpc-bind-addr".to_owned(), lightwalletd_rpc_address];
    ...
    let borrowed_arguments: Vec<&str> = arguments.iter().map(|string| string.as_ref()).collect();
    let mut lightwalletd = lightwalletd_dir.spawn_lightwalletd_child(&borrowed_arguments)?;
